    ...
    <aside className="w-24 lg:w-64 glass-panel border-r border-white/5 flex flex-col hidden md:flex z-50 shrink-0">
      ...
      <nav className="flex-1 px-4 py-8 space-y-4">
        ...
        <Link className={`
                ...
                ${currentPath === '/mod' ? 'bg-white/5 border border-white/10' : 'hover:bg-white/5'}
              `} to="/mod" data-discover="true" {...(currentPath === '/mod' ? { 'aria-current': 'page' } : {})}>
          ...
          <span className={`font-rajdhani font-bold tracking-widest text-sm hidden lg:block ${currentPath === '/mod' ? 'text-white' : 'text-slate-500'}`}>KIỂM DUYỆT</span>
          {currentPath === '/mod' && <div className="absolute left-0 w-1 h-8 bg-[#00F2EA] rounded-r-full shadow-[0_0_10px_#00F2EA]"></div>}
        </Link>
        ...
        <Link className={`
              ...
              `} to="/dashboard" data-discover="true" {...(currentPath === '/dashboard' ? { 'aria-current': 'page' } : {})}>
          ...
          <span className={`font-rajdhani font-bold tracking-widest text-sm hidden lg:block ${currentPath === '/dashboard' ? 'text-white' : 'text-slate-500'}`}>DASHBOARD</span>
        ...
        <Link className={`
          ...
          <span className={`font-rajdhani font-bold tracking-widest text-sm hidden lg:block ${currentPath === '/offices' ? 'text-white' : 'text-slate-500'}`}>QUẢN LÝ QUÁN</span>
        ...
        <Link className={`
              ...
              `} to="/notifications" data-discover="true" {...(currentPath === '/notifications' ? { 'aria-current': 'page' } : {})}>
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={currentPath === '/notifications' ? "2" : "1.5"} strokeLinecap="round" strokeLinejoin="round" className={`transition-all duration-300 ${currentPath === '/notifications' ? 'text-[#FFD700] drop-shadow-[0_0_8px_rgba(255,215,0,0.8)]' : 'text-slate-500'}`}>
            <path d="M12 2l9 5v10l-9 5-9-5V7z"></path>
            <circle cx="12" cy="10" r="3"></circle>
            <path d="M16 18c0-2-2-3-4-3s-4 1-4 3"></path>
          </svg>
          <span className={`font-rajdhani font-bold tracking-widest text-sm hidden lg:block ${currentPath === '/notifications' ? 'text-white' : 'text-slate-500'}`}>THÔNG BÁO</span>
      ...
      <div className="p-4 border-t border-white/5 flex flex-col items-center lg:items-start">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div className="hidden lg:block">
            <p className="text-xs font-bold font-rajdhani text-white truncate w-32 uppercase">{user?.fullName || "Admin Hệ Thống"}</p>
